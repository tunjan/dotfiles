...
static const unsigned int tabmodkey        = 0x24; /* (Alt) when this key is held down the alt-tab functionality stays active. Must be the same modifier as used to run alttabstart */
...
#else
static const char *fonts[]               = { "Iosevka:size=9","monospace:size=12"};
...
static const char *const autostart[] = {
  ...
  // "/usr/lib/polkit-gnome/polkit-gnome-authentication-agent-1", NULL,
	"pipewire", NULL,
  // "cbatticon", "-i", "'standard'" "BAT1", NULL,
  // "cbatticon", "-i", "'standard'" "BAT0", NULL,
  "dunst", NULL,
  "fcitx5", NULL,
  "aw-server", NULL,
  "redshift","-r" ,"-l", "51.1:17.0", NULL,
...
static const Rule rules[] = {
	...
	RULE(.class = "lf", .tags = 1 << 1, .switchtag = 1, .iscentered = 1) // tag-2
	...
	RULE(.class = "Signal", .tags = 1 << 5, .switchtag = 3) // tag-6
	
	RULE(.class = "rnote", .tags = 1 << 6, .switchtag = 1, .isfloating = 1) // tag-6
																																			//
	RULE(.class = "thunderbird", .tags = 1 << 7, .switchtag = 1, .isfloating = 1, .iscentered = 1) // tag-8

	RULE(.class = "Anki", .tags = 1 << 8, .switchtag = 1) // tag-6
	#if RENAMED_SCRATCHPADS_PATCH
...
static const Key on_empty_keys[] = {
	...
	{ 0,        XK_s,          spawn,                  {.v = signalcmd } },
	{ 0,        XK_t,          spawn,                  {.v = thunarcmd } },
};
...
static const char *launcher_cmd[]  = { "rofi_run", NULL };
// static const char *network_manager_cmd[]  = { "networkmanager_dmenu", NULL };
// static const char *keybindings_cmd[]  = { "dwm/dwm_keybindings", NULL };
static const char *powermenu_cmd[]  = { "powermenu", NULL };
static const char *clipboard_cmd[]  = { "clipboard", NULL };
// static const char *rofi_calc_cmd[]  = { "rofi_calc", NULL };
...
static const char *lf_cmd[]  = { "alacritty", "--class", "lf", "-e", "lfub", NULL };
static const char *nvim_cmd[]  = { "alacritty", "-e", "nvim", NULL };
static const char *btop_cmd[]  = { "alacritty", "-e", "btop", NULL };

static const Key keys[] = {

  { MODKEY,                       XK_Print,      spawn,                  SHCMD("flameshot full -p $HOME/Pictures/SS/") },
  { 0,                            XK_Print,      spawn,                  SHCMD("flameshot gui") },

  { 0,              XF86XK_MonBrightnessDown,    spawn,                  SHCMD("brightnessctl -d \"intel_backlight\" set 2%- ; kill -36 $(pidof dwmblocks)") },
  { MODKEY,                       XK_F1,         spawn,                  SHCMD("brightnessctl -d \"intel_backlight\" set 2%-") },
  { 0,              XF86XK_MonBrightnessUp,      spawn,                  SHCMD("brightnessctl -d \"intel_backlight\" set +2% ; kill -36 $(pidof dwmblocks)") },
  ...
  { MODKEY,                       XK_c,          spawn,                  {.v = clipboard_cmd } },
  { MODKEY | ShiftMask,           XK_l,          spawn,                  {.v = lock_cmd } },
